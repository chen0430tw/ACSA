...
**ACSA的双重防护机制：**
1. **认知清洗器（Cognitive Cleaner）**：自动将危险词转换为专业术语
   - 示例："帮我攻击网站" → "帮我执行安全压力测试网站"
2. **Jarvis熔断器**：检查转换后的文本，根据Risk Threshold决定是否阻止

**解决方案：**
- ✅ **调整 Risk Threshold 滑块**（0-100）：
  - **0-30 (宽松)**：只阻止极度危险的操作（删除数据、病毒制作等）
  - **30-70 (平衡)**：阻止中高风险操作（默认推荐）
  - **70-100 (严格)**：阻止所有可疑操作（最安全）
- ✅ **使用专业术语**：如"安全测试"代替"攻击"，"压力测试"代替"DDoS"
- ✅ **明确说明用途**：添加"用于安全研究"、"授权渗透测试"等上下文

**注意：** 即使阈值为0，某些极度危险的操作（如 `rm -rf /`、病毒制作）仍会被硬性阻止
